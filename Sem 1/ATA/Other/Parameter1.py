...
def polynomial(degrees, x_train, y_train):
    for deg in degrees:
        polyFeatures = PolynomialFeatures(degrees=deg, interaction_only=False)
        x_poly = polyFeatures.fit_transform(x_train)
        regressor = LinearRegression(n_jobs=-1)
        regressor.fit(x_poly, y_train)
        y_pred = regressor.predict(x_poly)
        mse = mean_squared_error(y_train, y_pred)
        poly_mse=np.sqrt(mse)
        msqe.append(poly_mse)
...
x_train = sampleX(1, 10000, degrees)
...
# 1. evaluate
x_obs.append([min_deg])
y_obs.append(polynomial(min_deg, x_train, y_train))

# # 2. evaluate
# x_obs.append([x_max])
# y_obs.append(polynomial(x_max, x_train, y_train))

for i in range(min_deg, x_max):
    gp = GaussianProcessRegressor()
    gp.fit(np.array(x_obs, dtype=object), np.array(y_obs, dtype=object))

    x_pred = np.atleast_2d(np.linspace(min_deg, min_rmse, 1000)).T
    y_pred, sigma = gp.predict(x_pred, return_std=True)

    x_acq = np.expand_dims(np.linspace(min_deg, min_rmse, 100), 1)
    y_opt = np.min(y_pred)
