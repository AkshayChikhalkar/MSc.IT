...
for n in range(0, 20, 5):
    for i in range(0, n):
        ...
        for name, classifier, param_dist in classifiers:
            ...
            i+=1
            print(i)
            scores = cross_val_score(classifier, X_train_selected, y_train, cv=5, scoring='accuracy')
            # Train the selected classifier on the whole training set
            classifier.fit(X_train_selected, y_train)

            trained_models[f'{name}_CV_{i}_{n}'] = classifier
            ...
            trained_models[f'{name}_GS_{i}_{n}'] = classifier
            ...
            trained_models[f'{name}_RS_{i}_{n}'] = classifier
...
import pickle
with open('trained_models.pkl', "wb") as json_file:
    pickle.dump(trained_models, json_file)
...
print(tabulate(results_CV_new.reset_index().applymap(lambda x: f"{x:.3f}" if isinstance(x, (int, float)) else x), headers='keys', tablefmt='pretty', showindex=False, floatfmt=".3f"))
...
predSVM=loaded_svm_model.predict(X_test)
report = classification_report(Y_test, predSVM, output_dict=True)
...
# %%
import re
with open('trained_models.pkl', "rb") as model_file:
    loaded_model = pickle.load(model_file)

for key, loaded_search_obj in loaded_model.items():
    # Parse the key to extract the components (name, i, n)
    components = re.split(r"_(.{2})_", key)
    name, type, rest = components[0], components[1], components[2]

    # Further split the rest to get i and n
    i, n = rest.split('_')

    # Access the best estimator from the loaded search object
    best_estimator = loaded_search_obj
